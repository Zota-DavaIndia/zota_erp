...
class AdminlteCustomPresenter extends Presenter
{
    ...
    {
        return '<div class="tw-flex-1 tw-px-2 tw-pt-1.5 tw-pb-3 tw-overflow-y-auto" id="side-bar">' . PHP_EOL;
    }
    ...
    {
        return '<div class="dava-menu-section-title">' . $item->title . '</div>';
    }
    ...
    protected function formatIcon($icon)
    {
        ...
        if (strpos($icon, '<svg') !== false) {
            return '<span class="menu-icon-wrap">' . $icon . '</span>';
        } else {
            // Assume it's a FontAwesome icon and return it wrapped in an <i> tag
            return '<span class="menu-icon-wrap"><i class="' . $icon . ' tw-text-base"></i></span>';
        }
    }
