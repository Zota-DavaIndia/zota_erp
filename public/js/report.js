$(document).ready(function() {
    ...
    if ($('table#sell_payment_report_table').length == 1) {
        var spr_cf = window.spr_custom_field_visible || { cf1: false, cf2: false, cf3: false, cf4: false };
        ...
        $('#sell_payment_report_table tbody').on('click', 'tr td.details-control', function() {
            ...
            if (row.child.isShown()) {
                ...
                tr.addClass('details');
                row.child(show_child_payments(row.data())).show();
                if (idx === -1) { spr_detail_rows.push(tr.attr('id')); }
            }
        });
    ...
    if ($('#spr_date_filter').length == 1) {
        ...
        $('#sell_payment_report_form #location_id, #sell_payment_report_form #customer_id, #sell_payment_report_form #payment_types, #sell_payment_report_form #customer_group_filter').change(function() {
            sell_payment_report.ajax.reload();
        });

        $('#spr_custom_field_1, #spr_custom_field_2, #spr_custom_field_3, #spr_custom_field_4').on('change', function() {
            sell_payment_report.ajax.reload();
        });
